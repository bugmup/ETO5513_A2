...
median_pivoted <- median_pivot %>%
  group_by(`Financial year`, State) %>%
  summarise(
    median_age = sort(rep(`HELP repayment income`,Count))[length(rep(`HELP repayment income`,Count))/2]
  )

median_pivoted <- mutate(median_pivoted, median_age = case_when(
    median_age== 9999 ~ "Up to $9,999",
    median_age==19999 ~ "$10,000 to $19,999", 
    median_age==29999 ~ "$20,000 to $29,999", 
    median_age==39999 ~ "$30,000 to $39,999", 
    median_age==49999 ~ "$40,000 to $49,999", 
    median_age==59999 ~ "$50,000 to $59,999", 
    median_age==69999 ~ "$60,000 to $69,999", 
    median_age==79999 ~ "$70,000 to $79,999", 
    median_age==89999 ~ "$80,000 to $89,999", 
    median_age==99999 ~ "$90,000 to $99,999", 
    median_age==109999 ~ "$100,000 to $109,999", 
    median_age==119999 ~ "$110,000 to $119,999", 
    median_age==129999 ~ "$120,000 to $129,999", 
    median_age==130000 ~ "$130,000 or more"
    ))

pivot_wide <- median_pivoted %>%
  pivot_wider(names_from = State, values_from = median_age)
